...
import 'package:contacts/ways/sqflite/pages/contacts_page.dart';
import 'package:contacts/ways/sqflite/pages/create_contact_page.dart';
import 'package:contacts/ways/sqflite/pages/deleted_contacts_page.dart';
import 'package:contacts/ways/sqflite/pages/logs_page.dart';
import 'package:contacts/ways/sqflite/pages/search_contacts_page.dart';
...
class SqfliteHomePageState extends State<SqfliteHomePage> {
  ...
  Widget sqfliteHomeWidget = new Container();
  ...
  Widget _sqfliteHomePage() {
    return new Stack(
      children: <Widget>[sqfliteHomeWidget, progressDialog],
    );
  }
  ...
  void handleNavigationDrawerClicks(String whatToDo) {
    setState(() {
      ...
      if (whatToDo != TAPPED_ON_HEADER) {
        Type type = sqfliteHomeWidget.runtimeType;
        if (title == whatToDo) {
          if (type == ContactPage) {
            ContactPage contactPage = sqfliteHomeWidget as ContactPage;
            contactPage.reloadContactList();
          } else if (type == DeletedContactsPage) {
            DeletedContactsPage deletedContactsPage =
                sqfliteHomeWidget as DeletedContactsPage;
            deletedContactsPage.reloadDeletedContacts();
          } else if (type == LogsPage) {
            LogsPage logsPage = sqfliteHomeWidget as LogsPage;
            logsPage.reloadLogs();
          }
        } else {
          title = whatToDo;
...
//------------------------------------------------------------------------------
          case EventConstants.READ_DELETED_CONTACTS_SUCCESSFUL:
            ...
            break;
          case EventConstants.READ_CONTACTS_UN_SUCCESSFUL:
            sqfliteHomeWidget = new DeletedContactsPage();
...
//------------------------------------------------------------------------------
          case EventConstants.READ_LOGS_SUCCESSFUL:
            sqfliteHomeWidget = new LogsPage(logs: eventObject.object);
            showSnackBar(SnackBarText.LOGS_LOADED_SUCCESSFULLY);
            break;
          case EventConstants.READ_LOGS_SUCCESSFUL:
            sqfliteHomeWidget = new LogsPage();
...
//------------------------------------------------------------------------------
          case EventConstants.NO_INTERNET_CONNECTION:
            sqfliteHomeWidget = NoContentFound(
